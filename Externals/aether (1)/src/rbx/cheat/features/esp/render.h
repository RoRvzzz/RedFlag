...
#include "../../../overlay/imgui/includes.h"
...
#define decl static auto
	...
	decl outlined_rect(ImVec2& pos, ImVec2& size, ImU32 col, float rounding = 0.f) {
		...
		if (globals::jail_esp) {
			draw->AddRect(
				ImVec2(rect_bb.Min.x - 2.f, rect_bb.Min.y - 2.f),
				ImVec2(rect_bb.Max.x + 2.f, rect_bb.Max.y + 2.f),
				IM_COL32(0, 0, 0, col >> 24),
				rounding
			);
		}
			draw->AddRect(
				ImVec2(rect_bb.Min.x - 1.f, rect_bb.Min.y - 1.f),
				ImVec2(rect_bb.Max.x + 1.f, rect_bb.Max.y + 1.f),
				ImGui::ColorConvertFloat4ToU32(ImVec4(globals::BoxOutlineColor[0], globals::BoxOutlineColor[1], globals::BoxOutlineColor[2], 1)),
				rounding
			);
	...
	decl cornered_rect(vec_ref pos, vec_ref size, ImU32 col) {
		...
		if (globals::fill_box) {
			...
			ImVec2 glow_offset = { 0, 0 };
			if (globals::boxGlow)
				draw->AddShadowRect({ X, Y }, { X + W, Y + H }, glow_color, globals::glow_size, glow_offset, ImDrawFlags_ShadowCutOutShapeBackground, 0.0f);

			ImU32 box_color = ImGui::ColorConvertFloat4ToU32(ImVec4(globals::color_5[0], globals::color_5[1], globals::color_5[2], 0.3f));
			draw->AddRectFilled({ X, Y }, { X + W, Y + H }, box_color);
		}
	...
	void health_bar(float max, float current, vec_ref pos, vec_ref size, float alpha_factor, float x_pad = 4.f, float bar_width = 4.f, bool blue = false)
	{
		...
		if (globals::HealthBasedColor) {
			if (health_percentage > 0.5f) {
		 	...
		 	ImVec4 clampedColor(
			 	std::clamp(globals::healthBar_Color[0], 0.0f, 1.0f),  // Red
			 	std::clamp(globals::healthBar_Color[1], 0.0f, 1.0f),  // Green
			 	std::clamp(globals::healthBar_Color[2], 0.0f, 1.0f),  // Blue
			 	std::clamp(globals::healthBar_Color[3] * alpha_factor, 0.0f, 1.0f)  // Alpha
			 );
	...
	decl text_Voided(vec_ref pos, const std::string& text, ImU32 col, ImFont* font, float font_size = 11.0f)
	{
	...
	decl draw_ray( RBX::Vector3& from,  RBX::Vector3& to, ImColor col, RBX::Vector2 dimensions, RBX::Matrix4x4 viewmatrix) {

		RBX::Vector2 screen_from = RBX::WorldToScreen(from, dimensions, viewmatrix); 
		RBX::Vector2 screen_to = RBX::WorldToScreen(to, dimensions, viewmatrix);     
		ImVec2 finalpos = ImVec2(screen_from.x, screen_from.y);
		ImVec2 finalpos2 = ImVec2(screen_to.x, screen_to.y);
	
		ImGui::GetBackgroundDrawList()->AddLine(finalpos, finalpos2, col);
	}
