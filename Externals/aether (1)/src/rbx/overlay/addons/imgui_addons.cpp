...
// Multiplication by another ImVec2
inline ImVec2 operator*(const ImVec2& lhs, const ImVec2& rhs) {
    return ImVec2(lhs.x * rhs.x, lhs.y * rhs.y);
}
...
bool ImAdd::CheckBox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(pos + ImVec2(style.FrameBorderSize, style.FrameBorderSize), pos + ImVec2(height, size.y) - ImVec2(style.FrameBorderSize, style.FrameBorderSize), GetColorU32(it_anim->second.Frame), style.FrameRounding);
    window->DrawList->AddRectFilledMultiColor(pos + ImVec2(style.FrameBorderSize, style.FrameBorderSize), pos + ImVec2(height, size.y) - ImVec2(style.FrameBorderSize, style.FrameBorderSize), GetColorU32(ImGuiCol_WindowShadow, 0), GetColorU32(ImGuiCol_WindowShadow, 0), GetColorU32(ImGuiCol_WindowShadow), GetColorU32(ImGuiCol_WindowShadow));
    if (style.FrameBorderSize) {
        window->DrawList->AddRect(pos, pos + ImVec2(height, size.y), GetColorU32(ImGuiCol_Border), style.FrameRounding, 0, style.FrameBorderSize);
    }

    if (label_size.x > 0)
    {
        window->DrawList->AddText(pos + ImVec2(height + style.ItemInnerSpacing.x, 0) + ImVec2(1, 1), GetColorU32(ImGuiCol_TitleBg), label);
        window->DrawList->AddText(pos + ImVec2(height + style.ItemInnerSpacing.x, 0) + ImVec2(-1, -1), GetColorU32(ImGuiCol_TitleBg), label);
        window->DrawList->AddText(pos + ImVec2(height + style.ItemInnerSpacing.x, 0) + ImVec2(-1, 1), GetColorU32(ImGuiCol_TitleBg), label);
        window->DrawList->AddText(pos + ImVec2(height + style.ItemInnerSpacing.x, 0) + ImVec2(1, -1), GetColorU32(ImGuiCol_TitleBg), label);
        window->DrawList->AddText(pos + ImVec2(height + style.ItemInnerSpacing.x, 0), GetColorU32(ImGuiCol_Text), label);
    }
...
bool ImAdd::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float width, const char* format)
{
    ...
    {
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize), grab_bb.Max + grab_padding - ImVec2(style.FrameBorderSize, style.FrameBorderSize), GetColorU32(it_anim->second.Grab), style.FrameRounding);
    }

    window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize), frame_bb.Max - ImVec2(style.FrameBorderSize, style.FrameBorderSize), GetColorU32(ImGuiCol_WindowShadow, 0), GetColorU32(ImGuiCol_WindowShadow, 0), GetColorU32(ImGuiCol_WindowShadow), GetColorU32(ImGuiCol_WindowShadow));
    ...
    window->DrawList->AddText(pos + ImVec2(w / 2 - CalcTextSize(value_buf).x / 2, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0) + ImVec2(1, 1), GetColorU32(ImGuiCol_TitleBg), value_buf);
    window->DrawList->AddText(pos + ImVec2(w / 2 - CalcTextSize(value_buf).x / 2, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0) + ImVec2(-1, -1), GetColorU32(ImGuiCol_TitleBg), value_buf);
    window->DrawList->AddText(pos + ImVec2(w / 2 - CalcTextSize(value_buf).x / 2, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0) + ImVec2(1, -1), GetColorU32(ImGuiCol_TitleBg), value_buf);
    window->DrawList->AddText(pos + ImVec2(w / 2 - CalcTextSize(value_buf).x / 2, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0) + ImVec2(-1, 1), GetColorU32(ImGuiCol_TitleBg), value_buf);
    window->DrawList->AddText(pos + ImVec2(w / 2 - CalcTextSize(value_buf).x / 2, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0), GetColorU32(ImGuiCol_Text), value_buf);
...
bool ImAdd::FrameRadio(const char* label, int* v, int current_id, const ImVec2& size_arg)
{
    ...
    // Animation State (For smooth color transitions)
    struct stColors_State {
        ImVec4 Frame;
        ImVec4 Border;
        ImVec4 Text;
    ...
    // Smooth color transition for animations
    it_anim->second.Frame = ImLerp(it_anim->second.Frame, colFrame, 1.0f / IMADD_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime);
    it_anim->second.Border = ImLerp(it_anim->second.Border, colBorder, 1.0f / IMADD_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime);
    it_anim->second.Text = ImLerp(it_anim->second.Text, colText, 1.0f / IMADD_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime);
    ...
    // Render the frame with a glowing effect when selected
    if (active) {
        // Apply glow effect when the radio button is selected
        window->DrawList->AddRectFilled(pos, pos + size, GetColorU32(it_anim->second.Frame), style.FrameRounding);
        window->DrawList->AddRect(pos, pos + size, GetColorU32(it_anim->second.Border), style.FrameRounding, 0, style.FrameBorderSize);
    }

    // Frame rendering with smooth animations
    window->DrawList->AddRectFilled(pos, pos + size, GetColorU32(it_anim->second.Frame), style.FrameRounding);
    if (style.FrameBorderSize > 0) {
        window->DrawList->AddRect(pos, pos + size, GetColorU32(it_anim->second.Border), style.FrameRounding, 0, style.FrameBorderSize);
    }
    ...
    window->DrawList->AddText(pos + ImVec2(size.x / 2 - label_size.x / 2, (size.y - label_size.y) / 2) + ImVec2(1, 1), GetColorU32(ImGuiCol_TitleBg), label);
    window->DrawList->AddText(pos + ImVec2(size.x / 2 - label_size.x / 2, (size.y - label_size.y) / 2) + ImVec2(-1, -1), GetColorU32(ImGuiCol_TitleBg), label);
    window->DrawList->AddText(pos + ImVec2(size.x / 2 - label_size.x / 2, (size.y - label_size.y) / 2) + ImVec2(-1, 1), GetColorU32(ImGuiCol_TitleBg), label);
    window->DrawList->AddText(pos + ImVec2(size.x / 2 - label_size.x / 2, (size.y - label_size.y) / 2) + ImVec2(1, -1), GetColorU32(ImGuiCol_TitleBg), label);
    window->DrawList->AddText(pos + ImVec2(size.x / 2 - label_size.x / 2, (size.y - label_size.y) / 2), GetColorU32(it_anim->second.Text), label);
...
bool ImAdd::FrameRadioColored(const char* label, int* v, int current_id, const ImVec2& size_arg, ImU32 text_color, ImU32 bg_color_1, ImU32 bg_color_2)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (held || hovered) {
        ...
        else bg_color_2 = bg_color_1;
    }

    RenderNavHighlight(bb, id);

    int vert_start_idx = window->DrawList->VtxBuffer.Size;
    window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_color_1, style.FrameRounding);
    ...
    window->DrawList->AddText(pos + ImVec2(size.x / 2 - label_size.x / 2, (size.y - label_size.y) / 2), text_color, label);
    PopStyleColor();
...
void ImAdd::BeginChild(const char* label, const ImVec2& size_arg)
{
    ...
    ImGui::BeginChild(label, size_arg, ImGuiChildFlags_None, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
    {
        ...
        pDrawList->AddRectFilled(pos, pos + size, GetColorU32(ImGuiCol_ChildBg));
        ...
        ImGui::BeginChild(string(label + string("#Main")).c_str(), ImVec2(), ImGuiChildFlags_Border, ImGuiWindowFlags_NoBackground);
    }
}
