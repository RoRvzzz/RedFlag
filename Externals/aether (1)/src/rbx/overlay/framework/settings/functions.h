...
public:
    ...
    void text(ImDrawList* draw_list, const char* text, const ImVec2& pos = ImVec2(0, 0), ImU32 col = IM_COL32(255, 255, 255, 255), const ImFont* font = nullptr, float font_size = 0.0f, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = nullptr);
   ...
   // void set_cursor_pos(const ImVec2& local_pos);
    void table_setup_column(const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImGuiID user_id = 0);
    ...
    void begin_child(std::string_view name, float x = 1, float y = 1, const ImVec2& size = ImVec2(0, 0));
    ...
    void multi_dropdown(std::string_view label, std::vector<bool>& variable, const std::vector<std::string>& labels, bool wo_label);
    ...
    bool keybind(const char* label, CKeybind* bind);

    bool KeybindTest(const char* label, int* key, int* mode);

    bool label_keybindA(const char* label, int* key, int* mode);

    bool label_keybind(const char* label, CKeybind* bind);

    bool label_keybindB(const char* label, CKeybind* bind);
