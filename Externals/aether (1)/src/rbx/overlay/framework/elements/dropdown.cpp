...
void c_gui::multi_dropdown(std::string_view label, std::vector<bool>& variable, const std::vector<std::string>& labels, bool wo_label = false)
{
    std::string preview = "-";
    ...
    if (begin_dropdown(label.data(), preview.c_str(), wo_label, static_cast<int>(labels.size()), 0))
    {
        for (size_t i = 0; i < labels.size(); i++)
        {
            bool current = variable[i];
            if (dd_selectable(labels[i].c_str(), &current))
