...
void c_gui::text(ImDrawList* draw_list, const char* text, const ImVec2& pos, ImU32 col, const ImFont* font, float font_size, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    if (!text || !*text)
        return;

    draw->text(draw_list, font, font_size, pos, col, text, nullptr, wrap_width, cpu_fine_clip_rect);
}

void c_gui::table_setup_column(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ImGui::TableSetupColumn(label, flags, init_width_or_weight, user_id);
}


void c_gui::table_headers_row()
{
...
{
    ImVec2 current_pos = GetCursorPos();
    SetCursorPos(ImVec2(x, current_pos.y));
}

void c_gui::set_cursor_pos_y(float y)
{
    ImVec2 current_pos = GetCursorPos();
    SetCursorPos(ImVec2(current_pos.x, y));
}
