...
bool c_gui::keybind(const char* label, CKeybind* bind)
{
    ...
    std::string active_key = bind->get_key_name();
...
bool c_gui::KeybindTest(const char* label, int* key, int* mode)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool c_gui::label_keybindA(const char* label, int* key, int* mode)
{
    ...
    KeybindTest(label, key, mode);

    return true;
}
bool c_gui::label_keybind(const char* label, CKeybind* bind)
{
  //  draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), draw->get_clr(clr->widgets.text), label);
    gui->set_cursor_pos(ImVec2(GetWindowWidth() - elements->widgets.key_size.x - GetStyle().WindowPadding.x, GetCursorPos().y));
    gui->keybind(label, bind);

    return true;
}
bool c_gui::label_keybindB(const char* label, CKeybind* bind)
{
      draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), draw->get_clr(clr->widgets.text), label);
    gui->set_cursor_pos(ImVec2(GetWindowWidth() - elements->widgets.key_size.x - GetStyle().WindowPadding.x, GetCursorPos().y));
    gui->keybind(label, bind);
