...
private:
    ...
    ID3D11ShaderResourceView* createTextureFromWebP(const std::vector<uint8_t>& imageData, int& width, int& height);
...
public:
    ...
    // Helper method to create texture from file path
    ImTextureID createTextureFromFile(const std::string& filepath, int& width, int& height);
