...
ID3D11ShaderResourceView* AvatarManager::createTextureFromWebP(const std::vector<uint8_t>& imageData, int& width, int& height) {
	if (imageData.empty() || !d3d_device) {
		return nullptr;
	}
	...
	hr = pStream->InitializeFromMemory((BYTE*)imageData.data(), static_cast<DWORD>(imageData.size()));
	if (FAILED(hr)) {
		pStream->Release();
		pFactory->Release();
		return nullptr;
	...
	hr = pConverter->CopyPixels(NULL, stride, bufferSize, pixels.data());
	if (FAILED(hr)) {
		pConverter->Release();
		pFrame->Release();
		pDecoder->Release();
	...
	// Flip image vertically (WIC loads images top-to-bottom, DirectX expects bottom-to-top)
	std::vector<uint8_t> flipped_pixels(bufferSize);
	for (UINT y = 0; y < height; y++) {
		UINT src_row = y;
		UINT dst_row = height - 1 - y;
		memcpy(flipped_pixels.data() + dst_row * stride, pixels.data() + src_row * stride, stride);
	}
	...
	D3D11_SUBRESOURCE_DATA subResource = {};
	subResource.pSysMem = flipped_pixels.data();
...
ImTextureID AvatarManager::createTextureFromFile(const std::string& filepath, int& width, int& height) {
	if (!d3d_device) return nullptr;
	
	// Check if it's a WebP file
	std::string lower_path = filepath;
	std::transform(lower_path.begin(), lower_path.end(), lower_path.begin(), ::tolower);
	bool is_webp = (lower_path.length() >= 5 && lower_path.substr(lower_path.length() - 5) == ".webp");
	...
	// Use WIC for WebP, stb_image for other formats
	if (is_webp) {
		ID3D11ShaderResourceView* srv = createTextureFromWebP(buffer, width, height);
		return (ImTextureID)(intptr_t)srv;
	} else {
		// Use existing createTextureFromBytes method for other formats
		return createTextureFromBytes(buffer, width, height);
	}
}
