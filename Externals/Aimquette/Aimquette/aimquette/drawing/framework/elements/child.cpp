...
bool begin_child_ex(const char* name, ImGuiID id, int x, int y, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
{
    ...
    draw->rect_filled(parent_window->DrawList, parent_window->DC.CursorPos, parent_window->DC.CursorPos + size, draw->get_clr(clr->window.background_two));
    // Draw inner background starting from accent line area (y=2) - draw on parent to cover accent lines
    draw->rect_filled(parent_window->DrawList, parent_window->DC.CursorPos + ImVec2(0, 2), parent_window->DC.CursorPos + size, draw->get_clr(clr->window.background_one));
    ...
    // Set child window background to transparent so our custom drawing shows through
    PushStyleColor(ImGuiCol_ChildBg, IM_COL32(0, 0, 0, 0));

    // Begin into window
    const bool ret = gui->begin(temp_window_name, NULL, window_flags);

    // Background is already drawn on parent window above, no need to draw again here

    // Restore style
    PopStyleColor(); // Pop the transparent background color
    ...
    // Set cursor position inside child window - no title text, so start at padding
    if (name && child_window->BeginCount == 1)
    {
        SetCursorPos(elements->widgets.padding);
    }
...
void c_gui::begin_child(std::string_view name, int x, int y, const ImVec2& size)
{
    ...
    // Allow scroll wheel for ESP and Theme boxes (hide scrollbar but allow mouse wheel scrolling)
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar;
    if (name != "ESP" && name != "Theme")
        window_flags |= ImGuiWindowFlags_NoScrollWithMouse;
    
    begin_child_ex(name.data(), id, x, y, size, ImGuiChildFlags_None, window_flags);
...
void c_gui::end_child()
{
    ...
    {
        ImGuiWindow* parent_window = g.CurrentWindow;
        ImVec2 full_child_size = child_size + ImVec2(0, var->window.titlebar);
        ImRect bb(parent_window->DC.CursorPos, parent_window->DC.CursorPos + full_child_size);
        ItemSize(full_child_size);
        
        // Add spacing after child window to prevent overlap - advance cursor position
        float spacing = elements->content.spacing.y * 2.0f; // Double spacing for better separation
        ItemSize(ImVec2(0, spacing));
        parent_window->DC.CursorPos.y += full_child_size.y + spacing;
        parent_window->DC.CursorPosPrevLine.y = parent_window->DC.CursorPos.y;
