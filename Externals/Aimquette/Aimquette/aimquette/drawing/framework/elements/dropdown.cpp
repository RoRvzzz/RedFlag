...
bool dd_selectable_ex(const char* label, bool active)
{
    ...
    // Prevent popup from auto-closing when this item is clicked
    // PushItemFlag to disable AutoClosePopups for multi-select dropdowns
    PushItemFlag(ImGuiItemFlags_AutoClosePopups, false);
    
    if (!ItemAdd(rect, id))
    {
        PopItemFlag(); // ImGuiItemFlags_AutoClosePopups
        return false;
    }

    // Use ButtonBehavior to register the item and get hover state
    bool hovered, held;
    ...
    // Pop the item flag we pushed earlier
    PopItemFlag(); // ImGuiItemFlags_AutoClosePopups
...
bool begin_dropdown_popup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags, int val)
{
    ...
    // Set popup size
    float w = bb.GetWidth();
    float calculated_height = (15.0f * val) + (3.0f * (val - 1)) + 5.0f;
    float max_height = 0.0f;
    
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)
    {
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x, w);
        max_height = g.NextWindowData.SizeConstraintRect.Max.y;
    }
    else
    {
        ...
        max_height = calc_max_popup_height(popup_max_height_in_items);
    }
    
    // Use max height if calculated height exceeds it (enables scrolling)
    float final_height = (max_height > 0.0f && calculated_height > max_height) ? max_height : calculated_height;
    SetNextWindowSize(ImVec2(w, final_height), ImGuiCond_Always);
    ...
    // Enable scrolling by removing AlwaysAutoResize flag (allows window to have fixed size and scroll when content exceeds it)
    // Add NoNavFocus to prevent popup from closing on click inside
    ...
    ImVec2 popup_size = GetWindowSize();
    draw->rect_filled(GetWindowDrawList(), popup_pos + ImVec2(2, 2), popup_pos + popup_size - ImVec2(2, 2), draw->get_clr(clr->window.background_one));
    // Add thin black border around popup
    draw->rect(GetWindowDrawList(), popup_pos, popup_pos + popup_size, draw->get_clr(ImColor(0, 0, 0)), 0, 0, 1.0f);
...
{
    // Ensure vector size matches count
    if (variable.size() != count)
    {
        variable.resize(count, 0);
    }
    ...
    if (begin_dropdown(label.data(), preview.c_str(), wo_label, count, 0))
    {
        for (auto i = 0; i < count; i++)
        {
            PushID(i);
            // Get current state
            bool is_selected = variable[i] != 0;
            // Check if clicked - this will prevent popup from closing
            if (dd_selectable_ex(labels[i], is_selected))
            {
                // Toggle the value immediately
                variable[i] = is_selected ? 0 : 1;
                MarkItemEdited(GetCurrentWindow()->GetID(label.data()));
            }
            PopID();
        }
        end_dropdown();
