...
// Key names array for keybind widget - using full names like Centrum
...
bool c_gui::keybind(const char* label, int* key, int* mode)
{
    ...
    if (*key != 0 && *key < 256) {
        active_key = keys[*key];
        // Convert to lowercase like Centrum
        std::transform(active_key.begin(), active_key.end(), active_key.begin(),
    ...
    if (mode != nullptr)
    {
        ...
        if (popup_open)
        {
            ...
            {
                // Match enum order: TOGGLE=0, HOLD=1, ALWAYS=2
                if (gui->selectable_ex("Toggle", 0 == *mode, ImVec2(60, 15)))
                    *mode = 0;

                if (gui->selectable_ex("Hold", 1 == *mode, ImVec2(60, 15)))
                    *mode = 1;
...
bool c_gui::label_keybind(const char* label, int* key, int* mode)
{
    ...
    return gui->keybind(label, key, mode);
}