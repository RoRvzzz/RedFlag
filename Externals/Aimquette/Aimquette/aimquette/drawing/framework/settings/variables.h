...
public:
	...
	struct
	{
		bool current_section[7] = {true, false, false, false, false, false, false}; // First section enabled by default
		const char* section_icons[IM_ARRAYSIZE(current_section)] = {"A", "B", "C", "D", "E", "F", "G"};

		float menu_alpha{ 0 };
		bool menu_opened{ true };
		int menu_key{ 45 };
		int menu_key_mode{ 0 }; // 0 = Toggle, 1 = Hold (matches keybind widget enum)
	} gui;
