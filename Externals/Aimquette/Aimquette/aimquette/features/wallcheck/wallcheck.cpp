...
namespace wallcheck {
    ...
    bool WallChecker::can_see(const Vector3& from, const Vector3& to) {
        ...
        const float unlock_distance = 0.1f; // Allow targets within 0.1 studs of wall

        for (const auto& wall : local_walls) {
            if (ray_intersects_obb_segment(from, to, wall)) {
                // Calculate distance from target point to wall
                float distance_to_wall = point_to_obb_distance(to, wall);
                
                // If target is within unlock distance, allow it
                if (distance_to_wall <= unlock_distance) {
                    continue; // Skip this wall, allow target
                }
                
                // Otherwise, wall blocks the target
                obstructed = true;
                break;
            }
        }
    ...
    float WallChecker::point_to_obb_distance(const Vector3& point, const roblox::wall& wall) {
        if (!is_valid_vector(point) || !is_valid_vector(wall.position) || !is_valid_vector(wall.size)) {
            return 1e6f; // Return large distance if invalid
        }

        // Transform point to wall's local space
        Matrix3 inverseRotation = wall.rotation.Transpose();
        Vector3 relativePoint = { point.x - wall.position.x, point.y - wall.position.y, point.z - wall.position.z };
        Vector3 localPoint = inverseRotation.multiplyVector(relativePoint);

        if (!is_valid_vector(localPoint)) {
            return 1e6f;
        }

        // Calculate half-size of OBB
        const float epsilon = 1e-6f;
        Vector3 halfSize = { wall.size.x / 2.0f, wall.size.y / 2.0f, wall.size.z / 2.0f };

        // Clamp point to OBB bounds to find closest point on OBB
        Vector3 closestPoint;
        closestPoint.x = std::max(-halfSize.x, std::min(halfSize.x, localPoint.x));
        closestPoint.y = std::max(-halfSize.y, std::min(halfSize.y, localPoint.y));
        closestPoint.z = std::max(-halfSize.z, std::min(halfSize.z, localPoint.z));

        // Calculate distance from point to closest point on OBB
        float dx = localPoint.x - closestPoint.x;
        float dy = localPoint.y - closestPoint.y;
        float dz = localPoint.z - closestPoint.z;
